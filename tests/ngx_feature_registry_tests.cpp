...
int main() {
    ...
    const auto failed = registry.RecordCreate(
        {0x10, 1, NgxFeatureKind::SuperResolution, false});
    assert(!failed);
    assert(registry.Size() == 0);
    assert(!registry.Lookup(1, 0x10));

    assert(!registry.RecordCreate(
        {0, 1, NgxFeatureKind::SuperResolution, true}));
    assert(!registry.RecordCreate(
        {0x10, 0, NgxFeatureKind::SuperResolution, true}));
    assert(!registry.RecordCreate(
        {0x10, 1, static_cast<NgxFeatureKind>(255), true}));
    assert(registry.Size() == 0);

    const auto sr = registry.RecordCreate(
        {0x100, 1, NgxFeatureKind::SuperResolution, true});
    const auto fg = registry.RecordCreate(
        {0x101, 1, NgxFeatureKind::FrameGeneration, true});
    const auto rr = registry.RecordCreate(
        {0x102, 1, NgxFeatureKind::RayReconstruction, true});
    const auto unknown = registry.RecordCreate(
        {0x103, 1, NgxFeatureKind::Unknown, true});
    ...
    const auto oldIdentity = registry.RecordCreate(
        {0x300, 21, NgxFeatureKind::SuperResolution, true});
    const auto reusedIdentity = registry.RecordCreate(
        {0x300, 21, NgxFeatureKind::FrameGeneration, true});
    ...
    const auto beforeClear = registry.RecordCreate(
        {0x301, 21, NgxFeatureKind::RayReconstruction, true});
    registry.Clear();
    ...
    for (std::size_t i = 0; i < capacityTokens.size(); ++i) {
        const auto created = registry.RecordCreate(
            {0x1000 + i, 31, NgxFeatureKind::Unknown, true});
        assert(created);
        capacityTokens[i] = created.token;
    }
    assert(registry.Size() == NgxFeatureRegistry::kCapacity);
    assert(!registry.RecordCreate(
        {0xFFFF, 31, NgxFeatureKind::SuperResolution, true}));
    ...
    for (std::uint32_t i = 0; i < kLifecycleIterations; ++i) {
        const auto kind = (i & 1u) == 0
            ? NgxFeatureKind::SuperResolution
            : NgxFeatureKind::FrameGeneration;
        const auto created = registry.RecordCreate({0x400, 41, kind, true});
        assert(created);
        assert(registry.Lookup(41, 0x400).kind == kind);
        assert(registry.RecordRelease(created.token));
    }
    assert(registry.Size() == 0);

    const auto hotSr = registry.RecordCreate(
        {0x500, 51, NgxFeatureKind::SuperResolution, true});
    const auto hotFg = registry.RecordCreate(
        {0x501, 51, NgxFeatureKind::FrameGeneration, true});
    assert(hotSr && hotFg);
